...
export const AvatarCropModal: FC<AvatarCropModalProps> = ({open, cropImage, onCancel, onOk}) => {
    ...
    const handleInit = () => {
        ...
        const cropperInstance = new Cropper(image, {
            ...
            template: '<cropper-canvas background="true" scale-step="0.1" style="width: 100%;height: 100%;overflow: visible">\n' +
        ...
        cropperInstance.getCropperImage()?.$ready((image) => {
            ...
            const transform = cropperInstance.getCropperImage()!.$getTransform();
            let scale = 1;
            ...
            if (Math.abs(transform[0] - scale) > 0.001) {
                const newTransform = [scale, 0, 0, scale, transform[4], transform[5]];
                cropperInstance.getCropperImage()!.$setTransform(newTransform);
                setInitTransform(newTransform)
            ...
            cropperInstance.getCropperImage()?.addEventListener('transform', (event: any) => {
                ...
                if (scaleX1 === scaleX2 && scaleY1 === scaleY2) { //非缩放操作
                    return;
                } else {
                    ...
                    if (scaleX1 < scaleX_origin || scaleY1 < scaleY_origin) { //缩小到比初始化小
                        event.preventDefault();
                        setZoom(1)
                    } else if (scaleX1 > (scaleX_origin * 3 + 0.001) || scaleY1 > (scaleY_origin * 3 + 0.001)) { //放大到3倍
    ...
    return wrapSSR(
        <Modal open={open} className={classNames(prefixCls, hashId)}
               styles={{
                   content: {
                       padding: '10px'
                   }
        ...
        >
            <Space direction={'vertical'}>
                <div style={{
                    ...
                    <div ref={cropContainerRef} className={classNames(`${prefixCls}-cropper`, hashId)}
                         style={{width: 300, height: 300, position: 'relative'}}/>
                </div>
                <Flex gap={'small'} justify={'space-between'} align={'center'}>
                    <Button type={'text'} icon={<ZoomOutOutlined/>} disabled={zoom <= 1} onClick={() => {
                        if (zoom - 0.1 >= 1) {
                            handleZoomChange(zoom - 0.1)
                        }
                    }}/>
                    <Slider style={{width: '100%'}} defaultValue={zoom} value={zoom} min={1} max={3} step={0.1}
                            onChange={handleZoomChange}/>
                    <Button type={'text'} icon={<ZoomInOutlined/>} disabled={zoom >= 3} onClick={() => {
                        if (zoom + 0.1 <= 3) {
                            handleZoomChange(zoom + 0.1)
                        }
                    }}/>
                ...
                <Flex gap={'small'} justify={'center'} align={'center'}>
                    <Button type={`text`} icon={<RotateLeftOutlined/>} disabled={rotate <= -180} onClick={() => {
                        if (rotate - 1 >= -180) {
                            handleRotateChange(rotate - 1)
                        }
                    }}/>
                    <Slider style={{width: '100%'}} defaultValue={rotate} value={rotate} min={-180} max={180} step={1}
                            onChange={handleRotateChange}/>
                    <Button type={`text`} icon={<RotateRightOutlined/>} disabled={rotate >= 180} onClick={() => {
                        if (rotate + 1 <= 180) {
                            handleRotateChange(rotate + 1)
                        }
                    }}/>
                ...
                {/*    <Button onClick={() => {*/}
                {/*        cropper?.getCropperImage().$move(10, 10)*/}
                {/*    }}>确定</Button>*/}
                {/*    <Button onClick={() => {*/}
                {/*        cropper?.getCropperImage()?.$scale(1.1)*/}
                {/*    }}>确定1</Button>*/}
                {/*    <Button onClick={() => {*/}
                {/*        cropper?.getCropperImage()?.$rotate('45deg')*/}
                {/*    }}>确定2</Button>*/}
                {/*</div>*/}
            </Space>
